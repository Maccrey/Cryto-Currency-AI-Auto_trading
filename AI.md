...
- 실행 원장에서 약신호 매수 비중이 높고 손절 손실이 익절 수익을 압도하면 약신호 신규 진입/추가매수를 차단
...
- 약신호 포지션이 기대와 다르게 움직이면 부분 손절을 반복하지 않고 전량 청산한다.
- 익절 후 잔여 포지션은 손절선을 매수가와 왕복 수수료 위로 올려 브레이크이븐 이상을 보호한다.
