...
- 1회 진입 예상 손절 손실을 `MAX_DAILY_LOSS`의 25%와 `CAPITAL_RISK_PCT` 기반 투자금 비례 예산 중 더 큰 값으로 자동 제한
- 가격 손절 + 기대 불일치 손절
- 텔레그램 거래/재기동 알림
...
- 24시간 수익률 시계열에 체결 타입을 함께 저장해 실행 내역 매칭이 어긋나도 매도/손절 마커 표시 유지
...
- 하락장 신규 매수/추가매수 차단과 하락장·박스권→상승장 전환 확인 후 진입 완화
- 손절 후에는 확정 상승장, strong 이상 신호, 손절가 대비 회복 가격이 모두 확인될 때만 재진입
- 일반 매도 후 같은 가격대 재매수를 차단하고, 더 낮은 재진입 가격 또는 확정 상승 돌파가 있을 때만 매수 허용
...
- **[2026-06-16] 하락→상승 전환 확인 매수 개선**
  ...
  - weak 신호나 단순 박스권 반등은 여전히 차단해 하락장 재진입 오탐을 줄인다.

- **[2026-06-16] 손절률 기반 룰 개선: post-entry 손절 앞당김 / 반복 부분손절 제거**
  - 최근 매매 로그에서 손절성 청산이 gross 약 **-1.04%~-1.09%**, net 약 **-1.14%~-1.19%** 구간에 집중되고, 같은 포지션이 여러 번 부분손절되는 패턴을 확인했다.
  - post-entry 모멘텀 반전 손절 감지선을 gross **-1.0% → -0.8%**로 앞당겨 왕복 수수료 포함 손실이 약 -1%에 도달하기 전에 방어하도록 조정했다.
  - `STOP_LOSS_*` post-entry 청산은 신호 강도와 무관하게 전량청산하도록 변경해 같은 하락 포지션에서 반복 부분손절이 누적되는 문제를 줄였다.
  - 고정 hard stop은 기존 프로필별 손절률을 유지하며, 이번 변경은 진입 후 기대 불일치 손절의 실행 방식만 강화한다.

- **[2026-06-16] 매매 룰 방어형 최적화: 익절 하한 상향 / 손절 후 재진입 강화 / weak 완화 차단**
  - 최근 demo 체결 데이터 기준 평균 익절폭이 손절폭보다 작아 손익비가 불리한 점을 반영해 일반 익절 최소 순수익을 **0.12% → 0.5%**로 상향했다. 왕복 수수료 0.1%를 포함한 총 익절 하한은 약 **0.6%**다.
  - 박스권 익절도 너무 좁은 수익 실현을 줄이기 위해 최소 순수익을 **0.3% → 0.4%**로 상향했다. 왕복 수수료 포함 총 기준은 약 **0.5%**다.
  - 손절 후 재진입은 medium 회복 신호만으로 허용하지 않고, 확정 상승장과 strong 이상 신호가 함께 확인될 때만 허용한다.
  - 무거래 완화가 weak 신호 진입으로 이어지는 경로를 기본 차단해 하락장/박스권 오탐 구간의 불필요한 진입을 줄였다.
  - 검증: `pytest -q` 전체 테스트 **482개 통과**.

- **[2026-06-14] 매매 룰 개선: 재진입 쿨다운 분리 / 박스권 오탐지 방지 / 상승장 재진입 조건 완화**
  - 재진입 쿨다운 분리: 익절 후 180초 → **60초**, 손절 후 → **120초** (`AdaptiveCooldownReentryPolicy`)
  - POST_SELL 재진입에 `uptrend_continuation` 모드 신설: medium 이상 신호 + bull 1틱 확인 + 마지막 매도가 이상이면 재진입 허용 (가격 상승 중 재매수 차단 문제 해소)
  - 손절 후 재진입 완화: medium 신호 기준 회복 가격 1.2% → 0.6%, 확인 틱 2~3틱 → 1틱(relaxed)
  - 시장 상태 판단 강화: 하락장 판정을 OR 조건 → AND 조건으로 변경하여 상승장 내 일시 조정을 하락장으로 오분류하는 문제 해소
  - 박스권 오탐지 방지: 절대 레인지 현재가의 0.3% 미만이면 noise 처리하여 10원 미만 레인지의 박스권 차단 해소
  - 박스권 최소 레인지 강화: 현재가의 0.5% 미만 레인지는 박스권으로 인정하지 않음, `box_range` 미발행
...
- 현재 누적 로그에서는 2026-05-27 23:12:19+09:00 이후 2026-05-30 10:40:15+09:00까지 체결이 없고, 주요 차단은 `WEAK_ENTRY_HISTORICAL_LOSS_BLOCK`, `MARKET_STATE_BEAR_ENTRY_BLOCK`, `SIDEWAYS_WEAK_SCALE_IN_BLOCK`이었다. 단, A/B/C 섀도우에서 상승장 룰 B가 장기간 우세하므로 demo 상승장, 룰 B 우세, 약신호 점수 0.24 이상, 수수료 엣지 재평가 조건이 모두 맞을 때만 회복 진입을 허용한다. 이후 하락장 예외 매수는 제거되어, 현재 장세가 하락장인 동안에는 반등 후보 신호가 있어도 매수를 차단한다.
- 현재까지 누적된 학습 로그/매매 데이터를 분석해 가격 카드 장세 판정을 매매 로직에 도입했다. 최근 체결은 하락장 진입 비중이 높고 A/B/C 섀도우에서는 방어형 C가 손실을 가장 작게 유지했기 때문에, 하락장 진입은 차단하고 하락장·박스권에서 상승장으로 확인된 전환만 제한적으로 진입 완화에 사용한다.
