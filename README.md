...
- 당일 실현손익이 `MAX_DAILY_LOSS`에 도달하면 신규 진입과 추가 진입을 차단하고, 보유 포지션의 손절·익절만 계속 처리
- 가격 손절 + 기대 불일치 손절
- 텔레그램 매수/매도/손절 체결 알림
- 텔레그램 등록 시 매수/매도/손절 체결이 발생한 경우에만 문장형 한국어 알림
...
- **[2026-07-12] 개선 진단 로그 추가 및 학습 데이터 삭제 확장**
  - `fallback_leader_selected`, `variant_portfolio_snapshot`, `stop_loss_triggered`, `market_transition_detected`, `reentry_block_released`, `rule_performance_daily_summary` 6종 학습 이벤트 신규 추가.
  - Fallback Leader 선발 시 어떤 룰이 왜 선택됐는지, 당시 모든 룰의 수익률/손절률/손절수를 기록해 하락장 복구 패턴 분석에 활용한다.
  - 손절 발생 직전 시장 상태·신호·가격 맥락을 별도 이벤트로 기록해 손절 원인 분석을 강화한다.
  - 시장 전환 감지(bear→bull / bull→bear) 시점을 별도로 기록해 전환 타이밍 분석을 지원한다.
  - 재진입 차단 해제 시점과 그때의 시장 상태·가격을 기록해 재진입 적절성 분석에 활용한다.
  - 설정 화면의 학습 데이터 리셋/삭제 기능이 새 로그 파일(`variant-performance.jsonl`, `stop-loss-analysis.jsonl`)도 함께 처리하도록 확장.

- **[2026-07-11] NO_POSITIVE_RULE_LEADER_YET 영구 거래 정지 버그 수정**
  - 07-07 13:30 이후 4일간 거래 완전 정지 원인 파악: 하락장 4일 지속 → A~R 모든 룰 섀도 수익률 음수 → `leader_key=None` → 신규 거래 차단 → 섀도 개선 불가 → **자기강화 악순환**.
  - **Fallback Leader 시스템** 도입: 정상 승격 가능 룰이 없을 때 최고 성과 룰(음수여도 가장 손실이 적은 룰)을 임시 리더로 선발해 매매를 유지한다.
  - 초기 기동(서버 재시작 후 섀도 포트폴리오 0거래 상태)에서는 낙폭 최소 기준으로 가장 안전한 룰을 즉시 선발한다.
  - Fallback 모드에서는 매수 크기를 50% 축소해 보수적으로 운용한다. 정상 승격 룰 발생 시 자동 전환.
  - `MIN_PROMOTION_TRADES`: 20 → **10**으로 완화해 하락장에서도 더 빠른 정상 승격이 가능하다.

- **[2026-07-07] 데모 룰 A~R 18개 확장 + 하락장 방어 강화**
  - A~F 6개 룰을 G~R까지 18개로 확장해 다양한 시장 상황(급등형, 박스권형, 방어형, 모멘텀형 등)을 동시 시뮬레이션한다.
  - 하락장 4일 이상 지속 시 연속 손절 쿨다운(`CONSECUTIVE_STOP_LOSS_COOLDOWN_TICKS=200`) 적용.
  - 비상 전환(Emergency Fallback): 현재 룰 손절 2회 이상 + 정상 승격 후보 없을 때 낙폭 최소 방어 룰로 강제 전환.
  - 손절 발생 즉시 다른 룰 중 양수 수익 룰로 강제 스위칭(`stop_loss_forced_switch`).
...
- **[2026-06-18] 데모 다중 룰 양수 수익 승격 게이트**
  - 최근 청산 9건 중 손절 5건이었고, 손절폭 약 0.80%~0.87%가 익절폭 약 0.55%~0.60%보다 커 누적 손익이 악화되는 패턴을 확인했다.
  - 기존 A/B/C에 돌파확인형 D, 박스저점형 E, 자본보전형 F를 추가해 장세별 후보를 6개로 확장했다.
  - 모든 후보가 손실이면 가장 덜 손실 난 룰도 적용하지 않는다. 양수 누적수익, 양수 실현손익, 최소 20회 청산, Profit Factor 1 초과, 손절 비중 40% 이하를 모두 만족한 후보만 리더가 될 수 있다.
  - replay는 거래가 실제로 발생하고 최종 수익률이 0%를 초과할 때만 통과한다. 양수 replay와 양수 섀도우 룰이 확인되어 demo 변경이 적용된 뒤에는 후보별 내부 성과를 리셋해 새 룰 기준으로 다시 비교한다.
  - 대시보드는 A~F 6개 카드를 모두 표시한다. 파란 테두리는 현재 수익률이 가장 높은 참고 후보, 주황 테두리는 양수 수익·최소 표본·Profit Factor·손절 비중 검증을 통과해 실제 demo 진입 정책에 적용된 룰이다.
  - 후보 순위는 누적 손절 횟수의 절대값이 아니라 수익률을 우선 비교한다. 따라서 모두 손실이면 `-0.13%`가 `-0.68%`보다 높은 후보로 표시되지만 신규 룰로 적용되지는 않는다.
  - 양수 검증 룰의 최고 카드가 바뀌면 같은 자동매매 주기부터 해당 룰의 장세별 진입 허용 조건과 매수 배수를 demo 주문 판단에 반영한다.
...
- 앱 서버 시작 시 텔레그램으로 서버 상태와 대시보드·설정 화면 주소를 전송한다. 트레이딩 루프 시작·중지 상태는 자동 전송하지 않는다.
...
- 텔레그램 정기 리포트는 보내지 않는다. 서버 시작, 급등·급락 감지, 매수/매도/손절 체결 알림을 한국어 문장으로 발송하며, 매도 알림에는 매수가 대비 손익과 수익률을 포함한다.
- 다중 매매 룰의 적용 대상이 바뀌면 변경 전 룰 유형과 누적 수익률, 변경 후 룰 유형과 누적 수익률, 손절 강제 전환 또는 성과 승격 여부와 변경 근거를 텔레그램으로 알린다.
- 단기 모멘텀과 전체 관측 구간 모멘텀을 분리해 같은 상승률의 이중 가중을 막고, 연속 손실 횟수를 다음 진입의 장세 점수와 비중 축소에 반영한다. 룰 replay 수익에는 매수·매도 수수료와 보유분 예상 청산 수수료를 포함한다.
...
- 현재가 카드의 장세 판정은 최근 가격 변화율 `±0.1%` 안쪽만 박스권으로 보고, 그 이상은 상승장/하락장으로 더 민감하게 전환한다. 과거 상·하단 접촉 때문에 박스권 조건이 성립하더라도 288개 가격 로그의 선형 기울기가 뚜렷하고 단기 흐름이 반대로 급변하지 않았다면 상승장/하락장을 우선한다. 박스권이면 현재가 카드 아래에 `박스권 하단 / 상단` 가격을 표시한다.
...
- `Codex 자동 룰 개선 시작`은 누적 학습 로그의 마지막 체결 시각, 24시간 무거래 여부, 차단 사유, A~F 섀도우 결과를 함께 검토한다. 자동매매 루프도 `AUTO_RULE_UPDATE_NO_TRADE_HOURS` 기본 24시간 동안 체결이 없고 차단 로그가 계속 쌓이면 같은 파이프라인을 자동 시작한다.
...
- 코인거래소 시뮬레이션의 운용상태, 차단 사유, A~F 최근 액션은 내부 코드값 대신 한국어 라벨로 표시한다.
