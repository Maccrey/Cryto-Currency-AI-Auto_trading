...
기본 자동화 성향은 단타 중심 `scalping`이다. 현재가는 3초 주기로 관찰하지만, 업비트 KRW 마켓 수수료 0.05%의 왕복 비용과 최소 순엣지를 넘지 못하는 진입은 차단한다.
...
- 업비트 KRW 수수료 0.05% 기준 단타 순엣지 게이트
- 1회 진입 예상 손절 손실을 `MAX_DAILY_LOSS`의 25% 이내로 자동 제한
- 가격 손절 + 기대 불일치 손절
- 텔레그램 거래/재기동 알림
- 텔레그램 등록 시 06:00~24:00 현재 트레이딩 정기 리포트
