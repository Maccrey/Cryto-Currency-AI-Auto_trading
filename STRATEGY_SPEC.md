...
현재 실제 주문에 적용되어 구동 중인 리더 룰(applied_variant_key)에서 손절(Stop Loss)이 발생하는 순간, 최소 15회 거래량 제약 조건(`_promotion_eligible`)은 **우회(Bypass)**하지만, 미실현 평가손익 0.01% 등의 노이즈성 승격을 차단하기 위해 **최소 1회 이상 거래 완료(`trade_count >= 1`) 및 실현 손익 양수(`realized_pnl > 0.0`) 조건이 검증된 룰**로만 대표 룰을 강제 전환(스위칭)합니다.
