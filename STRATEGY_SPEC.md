...
모멘텀 구간은 서로 겹치더라도 동일하게 계산하지 않는다.
- `ret_1s`: 직전 관측 대비 최신 관측 수익률
- `ret_5s`: 최신 3개 관측 구간 수익률. 기본 3초 주기에서 약 5~6초 흐름을 나타낸다.
- `ret_30s`: 현재 보유한 전체 판단 히스토리의 시작 대비 최신 관측 수익률

이를 통해 같은 전체 구간 수익률이 단기·장기 모멘텀 점수에 중복 반영되어 진입 강도가 과대평가되는 것을 막는다.
...
최근 연속 손실 횟수는 실행 원장의 체결 손익으로 계산한다. 손실 매도가 이어지면 국면 점수와 risk-off 비중을 낮추고, 수익 매도가 발생하면 연속 손실 횟수를 0으로 초기화한다.
...
## 10. 데모 룰 A~R 다중 변형 동시 테스트 (18개 룰)
...
|---|---|---|---|---|---|---|
| A | 안정형 | 기본 신호 + 장세 민감 배수 + 전환 감지 | 1.00 | 1.00 | 1.50% | 0.80% |
| B | 추세형 | 상승장·하락→상승 전환에만 크게 진입 | 1.85 | 0.45 | 3.20% | 1.20% |
| C | 방어형 | 하락장 즉시 청산, 박스 하단(40%↓) + 전환 시 소량 진입 | 0.38 | 2.00 | 1.00% | 0.60% |
| D | 돌파확인형 | 전환 확정 또는 상승장 모멘텀 돌파 확인 후 진입 | 1.25 | 0.70 | 2.50% | 1.00% |
| E | 박스저점형 | 박스 하단(38%↓) 반등 또는 전환 구간 거래 | 0.72 | 1.70 | 1.20% | 0.70% |
| F | 자본보전형 | 강한 신호·전환 구간에서만 소량 진입, 낙폭 억제 최우선 | 0.32 | 2.20 | 1.80% | 0.80% |
| G | 스캘핑형 | 강한 신호에만 소량 진입, 빠른 익절 및 손절 누적 | 0.55 | 1.50 | 0.80% | 0.50% |
| H | 모멘텀형 | 최강 상승 모멘텀과 시장 압력 동반 상승 시 공격 진입 | 2.20 | 0.35 | 3.80% | 1.40% |
| I | 분할매수형 | 하락 지속 시 분할 매수, 반등 전환 시 분할 청산 | 0.65 | 1.35 | 1.40% | 0.80% |
| J | 역추세형 | 과매도 박스 하단 강한 반등 타겟, 타이트한 손절 | 0.90 | 1.80 | 1.60% | 0.70% |
| K | 변동성형 | 변동성 급등 구간 소량 진입, 빠른 손절 보호 | 0.48 | 1.95 | 1.10% | 0.60% |
| L | 하이브리드형 | 추세형(B)과 방어형(C)을 장세별 자동 혼합 운영 | 1.10 | 1.15 | 2.00% | 0.90% |
| M | 돌파추격형 | 상승장 강력 모멘텀 확인 시 빠르게 진입하여 단기 상승 추세 극대화 | 1.50 | 0.80 | 2.80% | 1.10% |
| N | 역변동성형 | 변동성이 높고 가격이 박스권 극단 영역 도달 시 역추세 반등 노림 | 0.60 | 1.50 | 1.30% | 0.80% |
| O | 공격추세형 | 상승 확정 구간 최대 가중치 진입, 하락 전환 시 단호하게 청산 | 2.00 | 0.40 | 4.50% | 1.60% |
| P | 추세장기형 | 넓은 SL로 큰 상승 추세를 길게 보유, 복리 수익 극대화 | 2.20 | 0.30 | 5.50% | 1.80% |
| Q | 변동적응형 | 단기 변동성에 따라 TP/SL을 실시간 조율, 불필요한 청산 방지 | 0.85 | 1.20 | 2.60% | 1.20% |
| R | 반등돌파형 | 하락세 진정 및 상승 반전 초입에 공격 진입, 큰 반등 수익 포착 | 1.70 | 0.55 | 3.20% | 1.00% |
...
현재 실제 주문에 적용되어 구동 중인 리더 룰(applied_variant_key)에서 손절(Stop Loss)이 발생하는 순간, 최소 20회 거래량 제약 조건(`_promotion_eligible`)은 **우회(Bypass)**하지만, 미실현 평가손익 0.01% 등의 노이즈성 승격을 차단하기 위해 **최소 1회 이상 거래 완료(`trade_count >= 1`) 및 실현 손익 양수(`realized_pnl > 0.0`) 조건이 검증된 룰**로만 대표 룰을 강제 전환(스위칭)합니다.
이는 하락장 또는 특정 급락 구간에서 잘못된 리더 룰에 의해 발생할 수 있는 연속 손실 노출을 방어하고, 동시에 검증되지 않은 룰로의 불안정한 스위칭을 예방하기 위한 안전 조치입니다.

### 10.5 초기 조기 승격(Early Promotion) 메커니즘
서버 초기 기동 시 `_applied_variant_key`가 `None`인 상태(어떤 실매매 룰도 적용된 적 없음)에서는, 최소 거래 횟수 기준을 기존 `MIN_PROMOTION_TRADES = 20`에서 **`1`로 완화**하여 가장 먼저 1회 완결 거래 후 실현 수익이 발생한 룰이 즉시 리더로 등극합니다.
- **목적**: 서버 재기동 후 20회 거래가 채워질 때까지 모든 실매매가 영구 차단되는 대기 상태를 방지
- **조건**: `trade_count >= 1`, `realized_pnl > 0`, `profit_factor > 1.0`, `stop_loss_rate <= 0.40`
- **이후**: 일단 리더 룰이 등록되면 이후부터는 기존 `MIN_PROMOTION_TRADES = 20` 기준이 다시 적용됨
...
### 10.7 승격 조건
- 양수 수익률 + 양수 실현손익
- 최소 20회 청산 (`MIN_PROMOTION_TRADES = 20`)
- Profit Factor > 1.0
- 손절 비중 ≤ 40%
...
- replay 자산 평가는 매수 수수료, 매도 수수료, 마지막 보유 수량의 예상 청산 수수료를 반영한 순자산 기준으로 계산한다.
