...
모든 실행 모드에서 학습 로그를 항상 축적하여 전략과 모델을 지속 개선한다.

초기 자동화 성향은 `scalping` 단타 중심이다. 단, 거래 횟수를 늘리는 것이 목적이 아니라 짧은 주기로 시장을 관찰하고, 업비트 KRW 마켓 거래 수수료 0.05%와 추가 순엣지를 넘는 신호만 실행하는 것을 목표로 한다.
...
- 이 로그를 데이터셋으로 변환해 전략과 AI 모델을 개선
...
단타 진입 조건:
- 예상 엣지 > 왕복 수수료 `TRADING_FEE_RATE * 2` + 최소 순엣지 `SCALPING_MIN_NET_EDGE_PCT`
- 기본값 기준 최소 요구 엣지: 0.10% + 0.08% = 0.18%
