...
| 값 | 표시 | 성향 | 고정 손절 |
|---|---|---|---:|
| `scalping` | 단타 | 가장 빠른 관찰과 낮은 순엣지 기준 | -3% |
| `short_term` | 단기 | 수분 단위 흐름 확인 후 진입 | -3% |
| `mid_term` | 중기 | 추세 지속성과 리스크 여유를 더 크게 반영 | -5% |
| `long_term` | 장기 | 가장 느리게 관찰하고 높은 기대값만 진입 | -10% |
...
손절률은 투자성향별 고정값만 사용한다. 신호 강도별 손절률 튜닝은 폐기하며, `.env`에 남아 있는 `STOP_LOSS_*` 값보다 프로필 고정값이 우선한다. Codex 룰 개선 파이프라인은 손절 파라미터 변경안을 생성하거나 적용할 수 없다.
