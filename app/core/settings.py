...
def load_settings(*, env_file: Path | None = None) -> AppSettings:
    ...
    payload = {
        ...
        "stop_loss_weak": float(_setting("STOP_LOSS_WEAK", str(profile_spec.stop_loss_by_signal()["weak"]), env_values)),
        "stop_loss_medium": float(_setting("STOP_LOSS_MEDIUM", str(profile_spec.stop_loss_by_signal()["medium"]), env_values)),
        "stop_loss_strong": float(_setting("STOP_LOSS_STRONG", str(profile_spec.stop_loss_by_signal()["strong"]), env_values)),
        "stop_loss_very_strong": float(_setting("STOP_LOSS_VERY_STRONG", str(profile_spec.stop_loss_by_signal()["very_strong"]), env_values)),
