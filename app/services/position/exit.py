...
class PositionExitService:
    ...
    def evaluate_and_execute(
        ...
        *,
        ...
        exit_quantity = resolved_exit["quantity"]
        exit_ratio = resolved_exit["exit_ratio"]
        # 급락으로 추적청산선을 건너뛴 경우 수수료 차감 손실을 구분한다.
        is_take_profit = post_entry.reason_code == "TAKE_PROFIT_TARGET_HIT" or (
            post_entry.reason_code == "TRAILING_STOP_TRIGGERED"
            and current_price * (1.0 - self._trading_fee_rate)
            >= position.entry_price * (1.0 + self._trading_fee_rate)
        )
        sell_executor = (
